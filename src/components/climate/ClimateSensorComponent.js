...
const ClimateSensorComponent = ({deviceMap}) => {
	return <div className="p-2 form-group w-100 d-flex light_slider mb-2">
		...
		{<div className="temp-display pe-1 ps-1 d-flex align-items-center me-2" style={getIndoorTempStyle(getTemp(deviceMap))}>{getFormattedTemp(getTemp(deviceMap))} / {getHumidity(deviceMap)}%</div>}
	</div>
};
