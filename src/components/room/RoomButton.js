...
class RoomButton extends React.Component {
      ...
      <Button
        onClick={() =>
          this.props.handleClick(
            this.props.room.id,
            RoomButton.isOn(this.props.room) ? 0 : 100
          )
        }
  ...
  static isLight(theDevice) {
    return (
      theDevice.category === "2" ||
      theDevice.category === "3" /*&& !theDevice.name.includes("Override")*/
    );
  }
  ...
  getBatteryWarningStyle(theRoom) {
    let aLowBatteries = theRoom.devices.find((theDevice) => {
      return (
        theDevice.name != null &&
        theDevice.name.endsWith("Battery") &&
        parseInt(theDevice.level, 10) < 40 &&
        parseInt(theDevice.level, 10) !== 0
      );
    });
