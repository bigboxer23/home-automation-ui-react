	...
	if (RoomButton.isFan(device))
		{
			return <div className="p-2 w-100 h-100 d-flex flex-wrap justify-content-center align-content-start light_slider mb-2">
						<div className="form-group w-100">
							<div className="w-100 d-flex"><label className="ml-4 w-100">{device.name}</label>
								<IOSSwitch  checked={device.level > 0} onChange={(event) => setDeviceStatus(device.id, event.target.checked)}/></div>
						</div>
					</div>
		}
		return (<div className="p-2 w-100 h-100 d-flex flex-wrap justify-content-center align-content-start light_slider mb-2">
