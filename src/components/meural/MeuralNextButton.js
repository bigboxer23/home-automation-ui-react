...
export default function MeuralNextButton(props) {
	...
	const generateAlertText = (status) => {
		return ("1" === status ? "Started Generating New Image" : "Fetching Next Image");
	};
	...
	return (
				<div>
					<Button onClick={handleClick} variant="" size="lg" className={"mb-3 m-1 position-relative d-flex justify-content-center"}>
						<i className={getIconStyle(props.device?.status)}/>
						<div className="position-absolute bottom w-100 m-2 ps-2 pe-2">{generateButtonText(props.device?.status)}</div>
					</Button>
