...
export default function MeuralShowInfoButton(props) {
	...
	return (
			<div>
				<Button onClick={() => handleClick(props.show)} variant="" size="lg" className={shouldDisplay(props.device?.status)}>
					<i className={"mdi mdi-" + getIcon(props.show)}/>
					<div className="position-absolute bottom w-100 m-2 ps-2 pe-2">{getText(props.show)}</div>
				</Button>
			</div>
	);
}