...
class GarageButton extends React.Component
{
	...
	render()
	{
		return <Button onClick={() => this.props.handleGarageClick(GarageButton.isDoorOpen(this.props.room) ? "Close" : "Open")} variant={this.getButtonStyle()} size="lg" className={"m-1 position-relative d-flex justify-content-center"}>
			<i className="mdi mdi-garage"/>
			<div className="temp-display pe-1 ps-1 position-absolute" style={getTempStyle(GarageButton.findGarageTemperature(this.props.room))} onClick={(event) => this.props.handleGarageMoreClick(event)}>{getFormattedTemp(GarageButton.findGarageTemperature(this.props.room))}</div>
			<div className="autoClose">{GarageButton.getAutoClose(this.props.room)}</div>
			<div className="position-absolute bottom w-100 m-2 ps-2 pe-2">{this.props.room.name}</div>
		</Button>;
	}
